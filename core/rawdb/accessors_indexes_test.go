...
func TestDeleteBloomBits(t *testing.T) {
	...
	for i := uint(0); i < 2; i++ {
		for s := uint64(0); s < 2; s++ {
			WriteBloomBits(db, i, s, params.TestUsageMainnetGenesisHash, []byte{0x01, 0x02})
	...
	check(0, 0, params.TestUsageMainnetGenesisHash, true)
	check(0, 0, params.RinkebyGenesisHash, true)

	// Check the existence of deleted data.
	DeleteBloombits(db, 0, 0, 1)
	check(0, 0, params.TestUsageMainnetGenesisHash, false)
	check(0, 0, params.RinkebyGenesisHash, false)
	check(0, 1, params.TestUsageMainnetGenesisHash, true)
	...
	check(0, 0, params.TestUsageMainnetGenesisHash, false)
	check(0, 0, params.RinkebyGenesisHash, false)
	check(0, 1, params.TestUsageMainnetGenesisHash, false)
	check(0, 1, params.RinkebyGenesisHash, false)

	// Bit1 shouldn't be affect.
	check(1, 0, params.TestUsageMainnetGenesisHash, true)
	check(1, 0, params.RinkebyGenesisHash, true)
	check(1, 1, params.TestUsageMainnetGenesisHash, true)
